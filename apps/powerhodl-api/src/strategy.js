...
class MegaOptimalStrategy {
    constructor() {
        ...
        this.tradingEngine = new TradingEngine(this.parameters);
     ...
     * NOW USES UNIFIED TRADING ENGINE
     */
    getSignal(currentRatio, historicalRatios) {
        // Use a dummy portfolio for signal-only analysis
        const dummyPortfolio = { ethAmount: 5000, btcAmount: 5000 };
        
        // Use the unified trading engine
        const decision = this.tradingEngine.makeTradeDecision(
            currentRatio, 
            historicalRatios, 
            dummyPortfolio
        );
        ...
        return {
            ...
            strength: decision.signalStrength,
            shouldTrade: decision.shouldTrade,
            confidence: decision.signalConfidence,
            reasoning: decision.signalReasoning
        };
    }

    /**
     * Generate trading signal - uses unified engine for consistency
     */
    generateSignal(data) {
        // Extract data in various possible formats
        let currentRatio, historicalRatios, portfolio;
        
        if (Array.isArray(data)) {
            // Data is an array of historical points
            const latestData = data[data.length - 1];
            currentRatio = latestData.close;
            historicalRatios = data.map(d => d.close);
            portfolio = { ethAmount: 5000, btcAmount: 5000 }; // Default portfolio
        } else if (data.currentRatio) {
            // Data is an object with current ratio and portfolio
            currentRatio = data.currentRatio;
            historicalRatios = data.historicalRatios || [];
            portfolio = data.portfolio || { ethAmount: 5000, btcAmount: 5000 };
        } else {
