...
export class ModernMetaMaskService {
    ...
    async initializeModernMetaMask() {
        try {
            ...
            const isServerless = process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME || process.env.LAMBDA_RUNTIME_DIR;
            
            if (isServerless) {
                // Use direct ethers.js with private key for serverless
                await this.initializeServerlessSigner();
                return;
            }
            
            // Initialize MetaMask SDK for browser environments
    ...
    /**
     * Initialize serverless signer using ethers.js directly
     */
    async initializeServerlessSigner() {
        this.logger.info('🚀 Initializing serverless trading mode');
        
        if (!this.privateKey) {
            this.logger.warn('⚠️ No private key found - running in simulation mode');
            this.isSimulationMode = true;
            return;
        }
        
        // Use the existing server-side wallet initialization
        await this.initializeServerSideWallet();
        this.logger.info('✅ Serverless MetaMask service ready for automated trading');
    }
